...
// reduction is now delegated entirely to thrust::reduce, which avoids the
// malloc/cudaMalloc/cudaMemcpy/cudaFree churn of the previous hand-rolled
...
float PS_Box::sumDeviceArray(
    ...
    thrust::device_ptr<float> d_ptr = thrust::device_pointer_cast(d_dat);
    float totalSum = thrust::reduce(d_ptr, d_ptr + N, 0.0f);
