...
// Shared per-angle geometry/force/energy calculation used by both the
// force kernel (d_angles) and the stress/energy kernel (d_anglesStressEnergy).
//
// Given the positions and parameters for a single angle (particles ai-aj-ak),
// this computes the forces on the end particles (fi, fk), the connecting
// vectors (rij, rkj) with PBC applied, the cosine of the angle (cos_th), and
// the per-angle energy. Returns false for the early-continue cases (the two
// bond vectors too short, or cos_th out of range for the harmonic style), in
// which case the caller should skip this angle; returns true otherwise.
static __device__ inline bool d_computeAngle(
    ...
    const int atp,                  // angle type for this angle
    const float* L, const float* Lh,// [Dim]
    const int Dim,
    float* fi, float* fk,           // [3] out: forces on end particles
    float* rij, float* rkj,         // [3] out: connecting vectors (PBC applied)
    float& cos_th,                  // out: cosine of the angle
    float& energy ) {               // out: per-angle energy
    ...
    // Harmonic angle potential forces
      ...
      energy = angle_k[atp] * dtheta * dtheta;
...
__global__ void d_angles(
    ...
    const int ns, const int MAX_ANGLES, const int Dim ) {
        ...
        for ( int i=0 ; i < n_angles[ind] ; i++ ) {
            ...
            float fi[3], fk[3], rij[3], rkj[3], cos_th, energy;
            if ( !d_computeAngle(x, ai, aj, ak, angle_k, angle_theta_eq,
                    angle_style, atp, L, Lh, Dim, fi, fk, rij, rkj, cos_th, energy) )
                continue;
